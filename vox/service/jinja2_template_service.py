...
class Jinja2TemplateService:
    ...
    def __init__(self, theme_path: str):
        logger.info("Initializing Jinja2 template environment using '%s' path...", theme_path)
        self.__environment: Environment = Environment(
            loader=FileSystemLoader(theme_path),
            autoescape=select_autoescape()
        )

    def render_content_using_template(self, template_name: str, context: dict[str, Any]) -> str:
