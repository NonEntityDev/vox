...
def prepare_generate_command(app: typer.Typer):
    ...
    def generate(source: Annotated[str, typer.Option(help = "Path to the source FrontMatter document.")],
                 ...
                 content_type: Annotated[str, typer.Option(help = "Optional content type being generated.")] = "none",
        ...
        source_content_type:str | None = str(source_content["type"]) if "type" in source_content.keys() else None
        override_content_type: str | None = None if content_type.lower().strip() == "none" else content_type
        ...
        source_raw_html: str = markdown(source_content.content, extensions=[
        ...
        try:
            ...
            logger.error("It was not possible to load the template '%s' from '%s' path due the following error: %s", template_name, theme, str(ex), exc_info=True)
            raise typer.Abort(-1)
